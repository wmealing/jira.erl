...
search_test(Config) ->
    ...
    {ok, Issues, _TotalResults} = jira:search(
        "project = " ++ ct:get_config(jira_project), 0, 25, ["summary"], State
    ),
    NewConfig = [{issues, Issues} | Config],
    {{Y, M, D}, Time} = calendar:universal_time(),
    {ok, _Updated} = jira:search(
        jira:jql([
            {project, eq, ct:get_config(jira_project)},
            'and',
            {updated, gt, {{Y, M, max(D - 1, 1)}, Time}}
        ]),
        ["summary"],
        State
    ),
